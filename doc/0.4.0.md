...
This is the documentation for version 0.4.0. [Other versions](/beamr/doc/)
...
Python 3.4 or newer is required. While Python 2.7 was originally supported, it is no longer since one of the dependencies [removed](https://github.com/yaml/pyyaml/commit/dc0c4c1441b3ffca1343fd80790ebd846cbc1305) it.
...
The intended use of the program requires `pdflatex` to be called internally; for this a number of `texlive` packages are required and can be installed from the system package manager, e.g.:
    ...
    apt install cm-super texlive-extra-utils texlive-fonts-recommended texlive-font-utils texlive-latex-base texlive-latex-extra texlive-pictures texlive-plain-generic

This is an updated list based on more recent changes in the package distribution of `texlive`. It may vary between systems, so if you get LaTeX errors please browse your package manager for options.
If the intention is simply to generate LaTeX sources for use in an external engine (e.g. Overleaf), this can be done without having `texlive` installed at all.
...
Plus ([[{+}]](https://github.com/dave-v/plus/tree/fresh-start)) is a markup language for graphics, under development as part of another project, thus this construct is experimental. In order for it to work, the `plus` executable must be callable from Beamr, e.g. placed on the PATH or a link to it placed in the same virtual environment.
...
- An image grid/strip with gaps doesn't align properly
  ...
  Horizontal spacing only works when the image grid has a known width. Vertical spacing may look different to what you expect when a completely empty row exists.
